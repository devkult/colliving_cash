...
class HomeService:
    ...
    def add_bill(self, home: Home, user: User, amount: int, title: str = 'Unnamed', description: str = '') -> Bill:
        if user not in home.users:
            raise UserNotFoundException(username=user.name)
        return home.register_bill(user=user, bill=Bill(amount=amount, title=title, description=description))
